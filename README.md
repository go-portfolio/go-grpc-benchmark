...
**go-grpc-benchmark** — это инструмент для проведения производительных тестов и анализа эффективности **gRPC-сервисов** на языке **Go**.  
Он объединяет нагрузочное тестирование, мониторинг (Prometheus) и трассировку (OpenTelemetry + Jaeger).

---

## 📘 О проекте

Репозиторий предназначен для измерения производительности и сравнения различных реализаций gRPC-сервисов.  
Проект позволяет:

- запускать нагрузочные тесты на сервер;
- измерять задержки, TPS, использование CPU/RAM;
- собирать метрики в **Prometheus**;
- отслеживать трассы запросов через **OpenTelemetry / Jaeger**.

---

## 🚀 Возможности

- 🔁 Многопоточные тесты клиента к gRPC-серверу  
- ⚙️ Настройка количества запросов, потоков и размеров сообщений  
- 📊 Сбор метрик Prometheus (задержки, размеры, ошибки, TPS)  
- 🔎 Распределённая трассировка через OpenTelemetry  
- 🧩 Поддержка сценариев нагрузки (`light`, `peak`, `constant`)  
- 🔧 Конфигурация и отладка в VS Code 
